...
//  TimeRangeSlice
//
//  Created by Quinn on 2018/10/11.
...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let time1 = CMTimeRange.init(start: CMTime.init(value: 1, timescale: 1), end: CMTime.init(value: 5, timescale: 1))
        let time2 = CMTimeRange.init(start: CMTime.init(value: 2, timescale: 1), end: CMTime.init(value: 4, timescale: 1))
        
        let ranges = CMTimeRange.sliceTimeRanges(for: time1, timeRange2: time2)
        print(ranges)
...
extension CMTimeRange {
    ...
    static func mixTimeRanges(minTimeRange: CMTimeRange, instersectionTimeRange: CMTimeRange, maxTimeRange: CMTimeRange) -> [CMTimeRange] {
        ...
        if minTimeRange.containsTimeRange(maxTimeRange) {
            // A1           A2
            //      B1B2
            var timeRanges: [CMTimeRange] = []
            ...
            let rightTimeRangeDuration = minTimeRange.end - instersectionTimeRange.end
            ...
            return timeRanges
        }else if minTimeRange == maxTimeRange {
            // A1   A2
            // B1   B2
